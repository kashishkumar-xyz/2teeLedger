...
- **FR-005**: The system MUST securely manage the database encryption key. The key will be stored in the Android Keystore, and used to unlock the SQLCipher database via the FFI layer.
...
- **NFR-003**: The Android application MUST be buildable using standard Gradle commands (via Android Studio), and will depend on the compiled Rust core library (`libledger.so`).
