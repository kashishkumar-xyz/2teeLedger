...
fn test_cli_add_transaction() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("add")
        .arg("--db-path")
        .arg(db_path) // Pass the same db_path
        .arg("--encryption-key")
        .arg(key) // Pass the same key
        .arg("--person")
        .arg("Alice")
        .arg("--amount")
        .arg("100")
    ...
    let count: i64 = conn.query_row("SELECT COUNT(*) FROM transactions_history", [], |row| {
        row.get(0)
    })?;
    assert_eq!(count, 1);
...
fn test_cli_backup_db() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("Backup successful"));
...
fn test_cli_restore_db() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("Restore successful"));
