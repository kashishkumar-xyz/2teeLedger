use ledger_lib::ffi::{
    add_transaction, get_balance, init_db, list_balances, list_transactions, open_db,
};
...
fn test_add_transaction_ffi() {
    ...
    let result = unsafe { init_db(db_path.as_ptr(), key.as_ptr()) };
...
fn test_list_transactions_ffi() {
    ...
    // Initialize DB
    let init_result = unsafe { init_db(db_path.as_ptr(), key.as_ptr()) };
    ...
    let list_result =
        unsafe { list_transactions(db_path.as_ptr(), key.as_ptr(), ptr::null(), ptr::null(), 0) };
    ...
    let alice_transactions: Value =
        serde_json::from_str(list_result_alice_str.to_str().unwrap()).unwrap();
...
fn test_list_balances_ffi() {
    ...
    // List balances
    let list_balances_result = unsafe { list_balances(db_path.as_ptr(), key.as_ptr()) };
...
fn test_get_balance_ffi() {
    ...
    let alice_balance_result =
        unsafe { get_balance(db_path.as_ptr(), key.as_ptr(), person_alice.as_ptr()) };
    ...
    let bob_balance_result =
        unsafe { get_balance(db_path.as_ptr(), key.as_ptr(), person_bob.as_ptr()) };
    ...
    let charlie_balance_result =
        unsafe { get_balance(db_path.as_ptr(), key.as_ptr(), person_charlie.as_ptr()) };
...
fn test_key_management_ffi_init_open() {
    ...
    assert_eq!(
        init_result_str.to_str().unwrap(),
        "Database initialized successfully"
    );

    // 2. Open DB with correct key
    let open_result_ok = unsafe { open_db(db_path.as_ptr(), key.as_ptr()) };
    let open_result_ok_str = unsafe { CString::from_raw(open_result_ok as *mut _) };
    assert_eq!(
        open_result_ok_str.to_str().unwrap(),
        "Database opened successfully"
    );

    // 3. Open DB with wrong key
    let open_result_err = unsafe { open_db(db_path.as_ptr(), wrong_key.as_ptr()) };
    let open_result_err_str = unsafe { CString::from_raw(open_result_err as *mut _) };
    assert!(open_result_err_str
        .to_str()
        .unwrap()
